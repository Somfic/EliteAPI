...
namespace EliteAPI.Status.Provider
{
    ...
    public class StatusProvider : IStatusProvider
    {
        ...
        {
            try { return await FindFile(journalDirectory, "Status.json"); }
            catch (Exception ex) { throw new StatusFileNotFoundException("Could not find the Status.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindMarketFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "Market.json"); }
            catch (Exception ex) { throw new MarketFileNotFoundException("Could not find the Market.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindModulesFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "ModulesInfo.json"); }
            catch (Exception ex) { throw new ModulesInfoFileNotFoundException("Could not find the ModulesInfo.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindCargoFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "Cargo.json"); }
            catch (Exception ex) { throw new CargoFileNotFoundException("Could not find the Cargo.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindShipyardFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "Shipyard.json"); }
            catch (Exception ex) { throw new ShipyardFileNotFoundException("Could not find the Shipyard.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindOutfittingFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "Outfitting.json"); }
            catch (Exception ex) { throw new OutfittingFileNotFoundException("Could not get the Outfitting.json file", ex); }
        }

        /// <inheritdoc />
        public async Task<FileInfo> FindNavRouteFile(DirectoryInfo journalDirectory)
        {
            try { return await FindFile(journalDirectory, "NavRoute.json"); }
